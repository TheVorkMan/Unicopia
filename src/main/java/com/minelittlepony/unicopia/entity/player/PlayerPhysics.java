...
public class PlayerPhysics extends EntityPhysics<PlayerEntity> implements Tickable, Motion, NbtSerialisable {
    ...
    /* NeoForge, unlike Fabric additionally checks if player is in Creative/Spectator or it's own `NeoForgeMod.CREATIVE_FLIGHT` attribute in
    `ServerPlayer.tick()` and updates `abilities.flying` constantly every tick, which creates a problem with implemented flight in mod that
    works in survival - it changes `abilities.flying` to true, and that's all. NeoForge, with mod loaded through Connector - would forcibly
    reset the flag back to `false` the moment it processes the next movement packet from the client. */
